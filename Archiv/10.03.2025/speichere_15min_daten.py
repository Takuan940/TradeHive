...
# Funktion zum Abrufen von 15-Minuten-Daten in 180-Tage-Blöcken
def lade_historische_daten(start_date, end_date):
    """ Lädt historische 15-Minuten-Daten in 180-Tage-Blöcken ohne Überschneidung. """
    all_data = []
    current_date = datetime.strptime(start_date, "%Y%m%d %H:%M:%S")

    while current_date < datetime.strptime(end_date, "%Y%m%d %H:%M:%S"):
        next_date = current_date + timedelta(days=180)  # Max. 180 Tage pro Abfrage
        ...
        print(f"🔄 Lade 15-Minuten-Daten von {current_date.strftime('%d.%m.%Y')} bis {next_date.strftime('%d.%m.%Y')}...")
        ...
        if bars:
            ...
            print(f"⚠️ Keine Daten für 15 Min von {current_date.strftime('%d.%m.%Y')} bis {next_date.strftime('%d.%m.%Y')}")
...
# Fehlende 15-Minuten-Intervalle auffüllen
df_optimierung['date'] = pd.to_datetime(df_optimierung['date'])
df_optimierung = df_optimierung.set_index('date').asfreq('15min')

df_test['date'] = pd.to_datetime(df_test['date'])
df_test = df_test.set_index('date').asfreq('15min')
...
df_optimierung.to_parquet('spy_optimierung_15min.parquet', index=True)
df_test.to_parquet('spy_testdaten_15min.parquet', index=True)

print("✅ 15-Minuten-Daten erfolgreich gespeichert!")
